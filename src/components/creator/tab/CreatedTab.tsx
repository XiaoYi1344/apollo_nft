...
const CreatedTab: React.FC<Props> = ({ ownedProducts, allActivities, activitiesLoading, walletMode, handleOpenEdit, openSellModal }) => {
  return (
    <Grid container spacing={3} mb={3}>
      {ownedProducts
        .filter((product, idx) => {
          ...
          return isMinted; // chỉ hiển thị NFT đã minted
        })
        .map((product, idx) => {
          const activities = allActivities[idx] ?? [];
                ...
                <CardContent sx={{ bgcolor: '#1a1a2e' }}>
                  ...
                  {activitiesLoading ? (
                    ...
                    <Stack direction="row" spacing={1} sx={{ mt: 1, flexWrap: 'wrap' }}>
                      <Tooltip title="Sản phẩm đã được mint onchain"><Chip label="Minted" size="small" sx={{ bgcolor: 'rgba(0,255,127,0.15)', color: '#00FA9A', fontWeight: 600 }} /></Tooltip>
                  ...
                  {walletMode && (
                    <Stack direction="row" spacing={1} sx={{ mt: 1 }}>
                      <Button
                        size="small"
                        variant="contained"
                        sx={{ background: 'linear-gradient(90deg,#8b5cf6,#7c3aed)', textTransform: 'none' }}
                        onClick={() => openSellModal(product.id, product.price ?? '0')}
                      >
                        Sell
                      </Button>
                      <Button
                        size="small"
                        variant="outlined"
                        sx={{ borderColor: '#7a3bff', color: '#b78eff', textTransform: 'none' }}
