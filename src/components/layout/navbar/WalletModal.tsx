...
const WalletModal: React.FC<WalletModalProps> = ({
  ...
  const [walletState, setWalletState] = useState<WalletState>('select');
  const [connectingWalletName, setConnectingWalletName] = useState<string>('');
  const [walletListOpen, setWalletListOpen] = useState(false); // modal list wallets
  const { authenticateWallet, logout } = useWalletAuth();
  ...
  const connectWallet = async (wallet: string) => {
    try {
      ...
      if (wallet === 'metamask') {
        ...
        setTimeout(() => {
          setWalletState('info');
          setAccount('0xPioneerWalletAddress');
        }, 1500);
      } else {
        toast.error('Wallet not supported');
        setWalletState('select');
            ...
            >
              ...
              {walletState === 'select' && (
                <>
                  ...
                  <Stack spacing={2}>
                    ...
                    >
                      <Image src="/icons/wallet.svg" alt="all-wallets" width={28} height={28} />
                      <span style={{ marginLeft: 8, fontWeight: 600, color: '#fff', fontSize: 16 }}>
                        All Wallets
                      </span>
                    </Button>
              ...
              {walletState === 'connecting' && (
                <Box className="flex flex-col items-center justify-center">
                  ...
                  <Button disabled startIcon={<span className="w-2 h-2 rounded-full bg-yellow-400" />}
                    sx={{ color:'#fff', borderColor:'#fff', textTransform:'none', fontWeight:600, backgroundColor:'rgba(255,255,255,0.1)', borderRadius:'12px', px:3, py:1 }}>
                    Waiting for confirmation from {connectingWalletName}
                  </Button>
                </Box>
              )}
              ...
              {walletState === 'disconnect' && (
                <>
                  ...
                  <Typography variant="h6" sx={{ fontWeight:700, mb:1 }}>Wallet Disconnected</Typography>
                  <Typography variant="body2" sx={{ color:'#bcb9d3', mb:4, fontSize:'14px' }}>
                    You have disconnected your wallet. Connect again to continue.
                  </Typography>
      ...
      <Modal open={walletListOpen} onClose={() => setWalletListOpen(false)}>
        <Box className="fixed inset-0 flex items-center justify-center backdrop-blur-sm bg-black/40">
          <Paper className="w-[350px] p-6 rounded-xl text-white">
            <Typography variant="h6" sx={{ mb: 3, textAlign:'center' }}>Select Wallet</Typography>
            <Stack spacing={2}>
              <Button onClick={() => { connectWallet('metamask'); setWalletListOpen(false); }}
              ...
              {['coinbase','walletconnect'].map((w) => (
                <Button key={w} disabled sx={{ display:'flex', alignItems:'center', gap:1.5, justifyContent:'flex-start', px:3, py:1.5, borderRadius:2, bgcolor:'#1a1230', opacity:0.6 }}>
                  <Image src={`/icons/${w}.svg`} width={28} height={28} alt={w}/> {w.charAt(0).toUpperCase()+w.slice(1)}
                </Button>
              ))}
            </Stack>
          </Paper>
