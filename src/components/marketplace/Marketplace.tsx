...
const Marketplace = () => {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      setLoading(false); // sau khi dữ liệu tải xong
  ...
  if (loading) {
    return (
      ...
      >
        <CircularProgress sx={{ color: '#d622da' }} />
      </Box>
    );
  }
  ...
  return (
    ...
    >
      ...
      <Box sx={{ px: { xs: 2, md: 6 }, pt: 10 }}>
        {/* Tiêu đề */}
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 5, fontWeight: 700 }}>
        ...
        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
          {/* FilterPanel */}
          <Box sx={{ flexBasis: { xs: '100%', sm: '30%', md: '25%' }, flexShrink: 0 }}>
            <FilterPanel statusSelected={statusSelected} setStatusSelected={setStatusSelected} />
          </Box>

          {/* NFTTable */}
          <Box sx={{ flexBasis: { xs: '100%', sm: '70%', md: '75%' }, flexGrow: 1 }}>
